...
const CategoryForm = () => {
    ...
    // jsonData хэвлэх
    useEffect(() => {
        const dataInJson = JSON.stringify(combinedData, null, 2); 
        setJsonData(dataInJson);
    }, [combinedData]);
    ...
    return (
        <div className="w-[1000px] p-6 bg-black bg-opacity-80 print-area">
            ...
            <div className='flex p-4 w-full'>
                ...
                <button className="bg-green-500 bg-opacity-80 text-black py-2 px-4 rounded mb-6 ml-2 no-print" onClick={handleExport}>
                    Export JSON
                </button>
            </div>
            ...
            <div className={`pl-3 w-full max-w-[950px] text-left  ${expandAll ? '' : 'overflow-y-auto max-h-[600px] h-[600px] no-print'}`}>
                {filteredData.length === 0 ? (
                    <p className="absolute top-0 text-lg text-indigo-100">Алдааны жагсаалт олдсонгүй...</p>
                ) : (
            ...
            {jsonData && (
                <div className="text-left w-full mt-6">
                    <h3 className="text-xl text-white">JSON Data:</h3>
                    <pre className="text-white bg-gray-900 p-4 rounded">
                        {jsonData}
                    </pre>
                </div>
            )}
