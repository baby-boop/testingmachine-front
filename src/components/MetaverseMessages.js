...
const MetaverseMessages = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const alertsResponse = await axios.get('http://localhost:8080/api/mv-message');
                setAlerts(alertsResponse.data);

                const timeoutResponse = await axios.get('http://localhost:8080/api/mv-timeout');
                setTimeoutData(timeoutResponse.data);

                const metaDataResponse = await axios.get('http://localhost:8080/api/metaverse');
                setMetaData(metaDataResponse.data);

                const metaTotalCountResponse = await axios.get('http://localhost:8080/api/mv-total');
        ...
        <div className="w-[1000px] p-2 bg-black bg-opacity-80 print-area">
            ...
            <div className="text-left w-full print-area">
                <p className="text-white text-base mb-1 no-print">
                    Нийт шалгасан индикатор тоо: <strong>{metaData.processCount} </strong>
                </p>
                ...
                <p className="text-white text-base mb-3 no-print">
                    Нийт ажлуулж чадаагүй индикатор тоо: <strong>{timeoutData.length }</strong>
                </p>
            </div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-start' }}>
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <span className="text-sm text-white print-area">
                                                индикатор ID: {alert.metaId}
                                            </span>
                                            <span className="text-sm text-white print-area">
                                                индикатор Code: {alert.metaCode}
                                            </span>
                                            <span className="text-sm text-white print-area">
                                                Алдаа: {alert.message}
                                            </span>
