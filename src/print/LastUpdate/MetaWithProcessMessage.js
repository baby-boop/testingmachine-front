...
function MyComponent() {
    ...
    const fetchData = async () => {
      try {
        const [headerRes, resultRes] = await Promise.all([
          axios.get('http://localhost:8080/metaprocess-header'),
          axios.get('http://localhost:8080/metaprocess-result'),
        ]);
  ...
  const getTypeColor = (statusColor) => {
    ...
    return colorMap[statusColor.toLowerCase()] || '#ff8c8c';
  };
                    ...
                    <table className="w-full table-fixed bg-white text-black border-collapse mb-6">
                      <thead>
                        <tr className={`text-white justify-center items rounded-md bg-[${getTypeColor(process.status)}]`}>
                          ...
                          <th className="py-2 border border-gray-400 w-[80%] py-2 border border-gray-400 w-[80%]">Алдааны тайлбар</th>
                        </tr>
                      </thead>
                      <tbody>
